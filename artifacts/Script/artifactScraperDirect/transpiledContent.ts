...
(function (ArtifactScraperDirect) {
    ...
    var artifactInfoTile = __spreadArray(['actionApplication', 'settings', 'actionType'], artifactInfoTitle, true);
    var artifactInfoAPI = __spreadArray(['apiGroup', 'apiType', 'paths'], artifactInfoPackage, true);
    var artifactInfoScript = __spreadArray(['jsscriptGroup', 'globalScripts', 'externalModules', 'entitySets', 'apis'], artifactInfoPackage, true);
    ...
    var artifactConnector = __spreadArray(['settings'], artifactInfoPackage, true);
    var artifactJob = __spreadArray(['scripts'], artifactInfoPackage, true);
    ...
    var artifactScrapers = [
        ...
        { artifactType: 'launchpad', repositoryName: 'launchpad', selectInfo: artifactInfoLaunchpad, artifactMapFn: mapLaunchpad,
            usingFn: [{ propertyExtractFn: function (x) { return x.cat; }, artifactType: "tile_group" }, mapLaunchpadApp] },
        { artifactType: 'tile_group', repositoryName: 'category', selectInfo: artifactInfoTitle, artifactMapFn: mapLaunchpad,
            usingFn: [{ propertyExtractFn: function (x) { return x.tilegroups; }, artifactType: "tile_group" }, { propertyExtractFn: function (x) { return x.tiles; }, artifactType: "tile" }] },
        ...
        { artifactType: 'api_group', repositoryName: 'api_group', selectInfo: artifactInfoPackage, artifactMapFn: mapInfoPackage,
            childrenFn: [{ propertyExtractFn: function (x) { return x.apis; }, artifactType: "api" }] },
        ...
        { artifactType: 'script_project', repositoryName: 'jsscript_group', selectInfo: artifactInfoPackage, artifactMapFn: mapInfoPackage,
            childrenFn: [{ propertyExtractFn: function (x) { return x.jsscripts; }, artifactType: "script" }] },
        { artifactType: 'script', repositoryName: 'jsscript', selectInfo: artifactInfoScript, artifactMapFn: mapInfoScript,
            usingFn: [mapScriptUsing, { propertyExtractFn: function (x) { return x.entitySets.map(function (x) { var _a; return (_a = x.id) === null || _a === void 0 ? void 0 : _a.toUpperCase(); }); }, artifactType: "table" },
        ...
        { artifactType: 'adaptive', repositoryName: 'reports', selectInfo: artifactInfoAdaptive, artifactMapFn: mapInfoPackage,
            usingFn: [{ propertyExtractFn: function (x) { return x.connectorid ? [x.connectorid] : []; }, artifactType: "connector" },
        ...
        { artifactType: 'job', repositoryName: 'script_scheduler', selectInfo: artifactJob, artifactMapFn: mapInfoPackage,
            usingFn: [{ propertyExtractFn: function (x) { return x.scripts.map(function (x) { return x.id; }); }, artifactType: "script" }] },
                        ...
                        if (scraper.usingFn || scraper.childrenFn) {
                            _loop_1 = function (artifact) {
                                ...
                                if (scraper.childrenFn) {
                                    ...
                                    targetArtifact.children = allChildren.reduce(function (acc, x) { return (__spreadArray(__spreadArray([], acc, true), x, true)); }, []); //mapArtifactResponseFn(resp.data).map(x => x.id);
                                }
                                if (scraper.usingFn) {
                                    var allUsing = [];
                                    ...
                                    targetArtifact.using = allUsing.reduce(function (acc, x) { return (__spreadArray(__spreadArray([], acc, true), x, true)); }, []); //mapArtifactResponseFn(resp.data).map(x => x.id);
                                }
                            };
