...
function getWhereUsedTree(objectId, parent, level, tree) {
  ...
  const whereUsed = modelwhereUsedData
    .getData()
    .find((x) => x.objectId === objectId);
  // find users
  const whereUsedTabData = whereUsed?.used_by.map((y) => {
    const artifact = modelartifactsData
      .getData()
      .find((z) => z.objectId === y.id);
    if (artifact) {
      return {
  ...
  if (whereUsedTabData) {
    for (const element of whereUsedTabData) {
      const recursive = isRecursive(tree, parent, element.objectId);
...
function getUsingTree(objectId, parent, level, tree) {
  ...
  const sourceArtifact = modelartifactsData
    .getData()
    .find((z) => z.objectId === objectId);
